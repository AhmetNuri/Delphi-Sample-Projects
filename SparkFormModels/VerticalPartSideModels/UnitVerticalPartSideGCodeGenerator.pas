unit UnitVerticalPartSideGCodeGenerator;
...
uses
  ...
  FMX.ListBox, FMX.Layouts, VerticalPartSideRequest;

type
  TformVerticalPartSideGCodeGenerator = class(TForm)
    Label1: TLabel;
    edtRadiusOfInside: TNumberBox;
    Label2: TLabel;
    edtRadiusOfOutside: TNumberBox;
    Label5: TLabel;
    ...
    Label4: TLabel;
    edtRadiusOfTool: TNumberBox;
    procedure btnSelectFileClick(Sender: TObject);
    procedure FormClose(Sender: TObject; var Action: TCloseAction);
  private
    request: TVerticalPartSideRequest;

    procedure Generate(model: TVerticalPartSideRequest);
  public
    { Public declarations }
  end;

var
  formVerticalPartSideGCodeGenerator: TformVerticalPartSideGCodeGenerator;
...
uses CadHelpers, ShapeType, CadModel, CadService, DxfManager, Point2D, Point3D,
  Point6D, VerticalPartSide, VerticalPartSideModels, CadShapes,
  PointSplitter, MinMax2D, ShapeAnchor, LengthOfTemplate, CamService,
  CamManager, CamModel, CamLetter, CamType, CamPair, CamBlock, AxisSpindle,
  AxisType, AxisC, AxisU, AxisLaser, AxisFixed, AxisHighLathe, AxisLowLathe,
  Move, Mirror;

procedure TformVerticalPartSideGCodeGenerator.FormClose(Sender: TObject;
...
procedure TformVerticalPartSideGCodeGenerator.Generate
  (model: TVerticalPartSideRequest);
var
  ...
  instance: TVerticalPartSide;
  cadInput: TVerticalPartSideAxisIn;
  cadOutput: TVerticalPartSideAxisOut;
  camInput: TCamModel;
  block: TCamBlock;
...
begin
  ...
  instance := TVerticalPartSide.Create;
  cadSrv := TDxfManager.Create;
  camSrv := TCamManager.Create;

  if not Assigned(model.History) then
    model.History := TList<TVerticalPartSideRequestDetail>.Create;
  ...
  begin
    ...
    cadInput := TVerticalPartSideAxisIn.Create;
    cadInput.RadiusOfInside := model.History[I].RadiusOfInside;
    cadInput.RadiusOfOutside := model.History[I].RadiusOfOutside;
...
procedure TformVerticalPartSideGCodeGenerator.btnSelectFileClick
  (Sender: TObject);
begin
  if odDXF.Execute then
  begin
    if not Assigned(request) then
      request := TVerticalPartSideRequest.Create;
    ...
    request.Detail := TVerticalPartSideRequestDetail.Create;
    ...
    request.Detail.SpeedOfMovement := 5000;
    request.Detail.SpeedOfMilling := 2000;
    request.Detail.SpeedOfApproach := 3500;
    request.Detail.LimitForGapOfTouchAngle := 5;
    request.Detail.SafetyDistance := 1;
    request.Detail.ApproachDistance := 0.5;
    request.Detail.RadiusOfInside := edtRadiusOfInside.Value +
      edtRadiusOfTool.Value;
    request.Detail.RadiusOfOutside := edtRadiusOfOutside.Value;
    request.Detail.MinLimitOfTouchAngle := edtMinAngleOfTouch.Value;
    request.Detail.MaxLimitOfTouchAngle := edtMaxAngleOfTouch.Value;
    request.Detail.IsContinuousAngle := true;
    request.Detail.IsStrictDirection := true;
    request.Detail.IsDiscrete := false;
    request.Detail.IsReversedAngleOfTemplate := true;
    ...
    request.Detail.Stretch.SwitchModeToTemplate;
    request.Detail.Stretch.OnX.SetArea(0, 100);
