unit UnitVerticalPartGCodeGenerator;
...
uses
  ...
  FMX.ListBox, FMX.Layouts, VerticalPartRequest;

type
  TformVerticalPartGCodeGenerator = class(TForm)
    Label1: TLabel;
  ...
  private
    request: TVerticalPartRequest;

    procedure Generate(model: TVerticalPartRequest);
  public
    { Public declarations }
  end;

var
  formVerticalPartGCodeGenerator: TformVerticalPartGCodeGenerator;
...
procedure TformVerticalPartGCodeGenerator.FormClose(Sender: TObject;
...
procedure TformVerticalPartGCodeGenerator.Generate(model: TVerticalPartRequest);
var
...
begin
  ...
  if not Assigned(model.History) then
    model.History := TList<TVerticalPartRequestDetail>.Create;
...
procedure TformVerticalPartGCodeGenerator.btnSelectFileClick(Sender: TObject);
begin
  if odDXF.Execute then
  begin
    if not Assigned(request) then
      request := TVerticalPartRequest.Create;
    ...
    request.Detail := TVerticalPartRequestDetail.Create;
    ...
    // request.Detail.Stretch.OnY.SetArea(0, 50);

    // request.Detail.OffsetOfTouchAngle.SwitchTypeToStatic;
    // request.Detail.OffsetOfTouchAngle.OffsetValue := 30;
