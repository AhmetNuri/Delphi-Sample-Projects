unit UnitHorizontalPartLinearGCodeGenerator;
...
uses
  ...
  FMX.ListBox, FMX.Layouts, HorizontalPartLinearRequest;

type
  TformHorizontalPartLinearGCodeGenerator = class(TForm)
    Label2: TLabel;
  ...
  private
    request: THorizontalPartLinearRequest;

    procedure Generate(model: THorizontalPartLinearRequest);
  public
    { Public declarations }
  end;

var
  formHorizontalPartLinearGCodeGenerator
    : TformHorizontalPartLinearGCodeGenerator;
...
uses CadHelpers, ShapeType, CadModel, CadService, DxfManager, Point2D, Point3D,
  Point6D, HorizontalPartLinear, HorizontalPartLinearModels, CadShapes,
...
procedure TformHorizontalPartLinearGCodeGenerator.FormClose(Sender: TObject;
...
procedure TformHorizontalPartLinearGCodeGenerator.Generate
  (model: THorizontalPartLinearRequest);
var
  ...
  instance: THorizontalPartLinear;
  cadInput: THorizontalLinearAxisIn;
  cadOutput: THorizontalLinearAxisOut;
  camInput: TCamModel;
  block: TCamBlock;
...
begin
  ...
  instance := THorizontalPartLinear.Create;
  cadSrv := TDxfManager.Create;
  camSrv := TCamManager.Create;

  if not Assigned(model.History) then
    model.History := TList<THorizontalPartLinearRequestDetail>.Create;
  ...
  begin
    ...
    cadInput := THorizontalLinearAxisIn.Create;
...
procedure TformHorizontalPartLinearGCodeGenerator.btnSelectFileClick
  (Sender: TObject);
begin
  if odDXF.Execute then
  begin
    if not Assigned(request) then
      request := THorizontalPartLinearRequest.Create;
    ...
    request.Detail := THorizontalPartLinearRequestDetail.Create;
