unit UnitVerticalPartSquareStoneSettingGCodeGenerator;
...
uses
  ...
  FMX.ListBox, FMX.Layouts, VerticalPartSquareStoneSettingRequest;

type
  TformVerticalPartSquareStoneSettingGCodeGenerator = class(TForm)
    Label1: TLabel;
  ...
  private
    request: TVerticalPartSquareStoneSettingRequest;

    procedure Generate(model: TVerticalPartSquareStoneSettingRequest);
  public
    { Public declarations }
  end;

var
  formVerticalPartSquareStoneSettingGCodeGenerator
    : TformVerticalPartSquareStoneSettingGCodeGenerator;
...
uses CadHelpers, ShapeType, CadModel, CadService, DxfManager, Point2D, Point3D,
  ...
  SquareStoneSetting, PointSplitter, MinMax2D, ShapeAnchor, LengthOfTemplate,
...
procedure TformVerticalPartSquareStoneSettingGCodeGenerator.FormClose
...
procedure TformVerticalPartSquareStoneSettingGCodeGenerator.Generate
  (model: TVerticalPartSquareStoneSettingRequest);
var
  ...
  SquareStoneSettingOut: TSquareStoneSettingOut;
  camInput: TCamModel;
  block: TCamBlock;
...
begin
  ...
  if not Assigned(model.History) then
    model.History := TList<TVerticalPartSquareStoneSettingRequestDetail>.Create;
  ...
  begin
    ...
    begin
      ...
      SquareStoneSettingOut := cadSrv.CreateSquareStoneSetting
        (model.LineDetail.StoneSettingRule);
      ALineStream := SquareStoneSettingOut.ALineStream;

      if SquareStoneSettingOut.Shapes.Count > 0 then
      begin
        cadSrv.ClearData;
        for shapeIndex := 0 to SquareStoneSettingOut.Shapes.Count - 1 do
        begin
          cadSrv.Data.AddShape(SquareStoneSettingOut.Shapes[shapeIndex]
            .Lines[0]);
          cadSrv.Data.AddShape(SquareStoneSettingOut.Shapes[shapeIndex]
            .Lines[1]);
          cadSrv.Data.AddShape(SquareStoneSettingOut.Shapes[shapeIndex]
      ...
      APointStream := SquareStoneSettingOut.APointStream;
    ...
    inc(I);
//    if Assign(splitter)      then
    FreeAndNil(splitter);
    if Assigned(SquareStoneSettingOut) then

    FreeAndNil(SquareStoneSettingOut);
  end;
...
procedure TformVerticalPartSquareStoneSettingGCodeGenerator.btnGenerateClick
  (Sender: TObject);
begin
  if not Assigned(request) then
    request := TVerticalPartSquareStoneSettingRequest.Create;
  ...
  request.LineDetail := TVerticalPartSquareStoneSettingRequestDetail.Create;
  ...
  request.LineDetail.StoneSettingRule.XDistance := edtXDistance.Value;
  request.LineDetail.StoneSettingRule.YDistance := edtYDistance.Value;

  if radioCountByDistance.IsChecked then
    request.LineDetail.StoneSettingRule.RecurrenceMode := ssrCountByDistance
  else
    request.LineDetail.StoneSettingRule.RecurrenceMode := ssrDistanceByCount;
  ...
  request.PointDetail := TVerticalPartSquareStoneSettingRequestDetail.Create;
  ...
  request.PointDetail.StoneSettingRule.XDistance := edtXDistance.Value;
  request.PointDetail.StoneSettingRule.YDistance := edtYDistance.Value;

  if radioCountByDistance.IsChecked then
    request.PointDetail.StoneSettingRule.RecurrenceMode := ssrCountByDistance
  else
    request.PointDetail.StoneSettingRule.RecurrenceMode := ssrDistanceByCount;
