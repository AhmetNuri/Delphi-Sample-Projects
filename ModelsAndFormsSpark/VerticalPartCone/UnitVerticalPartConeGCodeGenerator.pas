unit UnitVerticalPartConeGCodeGenerator;
...
uses
  ...
  FMX.ListBox, FMX.Layouts, VerticalPartConeRequest;

type
  TformVerticalPartConeGCodeGenerator = class(TForm)
    Label1: TLabel;
  ...
  private
    request: TVerticalPartConeRequest;

    procedure Generate(model: TVerticalPartConeRequest);
  public
    { Public declarations }
  end;

var
  formVerticalPartConeGCodeGenerator: TformVerticalPartConeGCodeGenerator;
...
uses CadHelpers, ShapeType, CadModel, CadService, DxfManager, Point2D, Point3D,
  Point6D, VerticalPartCone, VerticalPartConeModels, CadShapes,
...
procedure TformVerticalPartConeGCodeGenerator.FormClose(Sender: TObject;
...
procedure TformVerticalPartConeGCodeGenerator.Generate
  (model: TVerticalPartConeRequest);
var
  ...
  instance: TVerticalPartCone;
  cadInput: TVerticalPartConeAxisIn;
  cadOutput: TVerticalPartConeAxisOut;
  camInput: TCamModel;
  block: TCamBlock;
...
begin
  ...
  instance := TVerticalPartCone.Create;
  cadSrv := TDxfManager.Create;
  camSrv := TCamManager.Create;

  if not Assigned(model.History) then
    model.History := TList<TVerticalPartConeRequestDetail>.Create;
  ...
  begin
    ...
    cadInput := TVerticalPartConeAxisIn.Create;
    cadInput.LowXOfProjection := model.History[I].LowXOfProjection;
    cadInput.HighXOfProjection := model.History[I].HighXOfProjection;
    cadInput.DepthOfCone := model.History[I].DepthOfCone;
...
procedure TformVerticalPartConeGCodeGenerator.btnSelectFileClick
  (Sender: TObject);
begin
  if odDXF.Execute then
  begin
    if not Assigned(request) then
      request := TVerticalPartConeRequest.Create;
    ...
    request.Detail := TVerticalPartConeRequestDetail.Create;
    ...
    request.Detail.LowXOfProjection := edtLowX.Value;
    request.Detail.HighXOfProjection := edtHighX.Value;
    request.Detail.DepthOfCone := edtDepthOfCone.Value;
