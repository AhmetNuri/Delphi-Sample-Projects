unit UnitVerticalPartSpiralSurfaceGCodeGenerator;
...
uses
  ...
  FMX.ListBox, FMX.Layouts, VerticalPartSpiralSurfaceRequest;

type
  TformVerticalPartSpiralSurfaceGCodeGenerator = class(TForm)
    Label1: TLabel;
  ...
  private
    request: TVerticalPartSpiralSurfaceRequest;

    procedure Generate(model: TVerticalPartSpiralSurfaceRequest);
  public
    { Public declarations }
  end;

var
  formVerticalPartSpiralSurfaceGCodeGenerator
    : TformVerticalPartSpiralSurfaceGCodeGenerator;
...
uses CadHelpers, ShapeType, CadModel, CadService, DxfManager, Point2D, Point3D,
  ...
  SpiralSurface, PointSplitter, MinMax2D, ShapeAnchor, LengthOfTemplate,
  ...
  Move, Mirror, AxisLowLathe;

procedure TformVerticalPartSpiralSurfaceGCodeGenerator.FormClose
...
procedure TformVerticalPartSpiralSurfaceGCodeGenerator.Generate
  (model: TVerticalPartSpiralSurfaceRequest);
var
  splitter: TPointSplitter;
  cadSrv: ICadService;
  camSrv: ICamService;
  AStream: TStream;
  I, J, indexOfPoints, lineIndex: Integer;
  instance: TVerticalPartRadius;
  cadInput: TVerticalRadiusAxisIn;
  cadOutput: TVerticalRadiusAxisOut;
  spiralSurfaceOut: TSpiralSurfaceOut;
  camInput: TCamModel;
  block: TCamBlock;
...
begin
  ...
  if not Assigned(model.History) then
    model.History := TList<TVerticalPartSpiralSurfaceRequestDetail>.Create;
  ...
  begin
    ...
    begin
      ...
      spiralSurfaceOut := cadSrv.CreateSpiralSurface(model.Detail.SurfaceRule);
      AStream := spiralSurfaceOut.AStream;

      if spiralSurfaceOut.Lines.Count > 0 then
      begin
        for lineIndex := 0 to spiralSurfaceOut.Lines.Count - 1 do
        begin
          cadSrv.Data.AddShape(spiralSurfaceOut.Lines[lineIndex]);
        end;
      end;
    end;
    ...
    begin
      ...
      if (I = model.History.Count) and (spiralSurfaceOut.Lines.Count > 0) then
      begin
...
procedure TformVerticalPartSpiralSurfaceGCodeGenerator.btnGenerateClick
  (Sender: TObject);
begin
  if not Assigned(request) then
    request := TVerticalPartSpiralSurfaceRequest.Create;
  ...
  request.Detail := TVerticalPartSpiralSurfaceRequestDetail.Create;
  request.Detail.TypeOfAxis := atFixed;
  ...
  request.Detail.AngleOfArmRef := 90;
  request.Detail.MinLimitOfTouchAngle := -120;
  request.Detail.MaxLimitOfTouchAngle := 120;
  request.Detail.IsContinuousAngle := false;
  request.Detail.IsStrictDirection := true;
  request.Detail.IsDiscrete := false;
  request.Detail.IsReversedAngleOfTemplate := false;
  request.Detail.SurfaceRule.RuleValue := edtRuleValue.Value;

  if radioCountByDistance.IsChecked then
    request.Detail.SurfaceRule.RuleMode := ssmCountByDistance
  else
    request.Detail.SurfaceRule.RuleMode := ssmDistanceByCount;
